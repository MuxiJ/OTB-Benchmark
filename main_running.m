...
addpath(('G:\LearningAlgorithm\code\vlfeat-0.9.21\toolbox'));
vl_setup
...
evalType='TRE'; %'OPE','SRE','TRE'  OPE是一次成功率，TRE（20次）和SRE（12次）都是多次的
...
%finalPath = ['./results/results_' evalType '_CVPR13/'];% 输出：运行算法后此算法在每个序列的的mat文件的保存位置
finalPath = ['./results/results_' evalType '_lct2/' ];
